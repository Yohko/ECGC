...
function GC_plot_umolhrsemilog()
    global GC_usersetting
    eval(GC_usersetting); % load settings
    global input result

    figure('units','centimeters','position',[0 0 w_fig h_fig*2]);
    semilogy( input.runnum, result.COumolhr/input.area,'o-', 'linewidth', f_line);
    hold on;
    semilogy( input.runnum, result.CH4umolhr/input.area,'o-', 'linewidth', f_line);
    semilogy( input.runnum, result.C2H4umolhr/input.area,'o-', 'linewidth', f_line);
    %plot( input.runnum, result.C2H6umolhr/input.area,'o-', 'linewidth', f_line);
    semilogy( input.runnum, result.H2umolhr/input.area,'o-', 'linewidth', f_line);
    %plot( input.runnum, result.O2umolhr/input.area,'o-', 'linewidth', f_line);
    hold off;
...
%        print(sprintf('%s_umolhrsemilog.png',input.resultname),'-dpng', '-r600');
%        %print(sprintf('%s_umolhrsemilog.pdf',resultname),'-dpdf');
%    end
end
