...
function GC_exportXLS()
    ...
    if(input.GCandEC == 1)    
        ...
        xlsData = {'INDEX' 'Sorted INDEX' 'U vs. RHE' 'CO' 'CH4' 'C2H4' 'H2' 'Total' 'current' 'charge'};
        xlwrite(fileName, xlsData, sheetName, sprintf('%s1',GC_getXLScolumn(coloffset)));
        maxrows = length(input.runnum)+1;
        for i=2:maxrows
            xlsData = {
                sprintf('=IFERROR(IF(ISBLANK($%s%d),"",1)*ROW(),"")',GC_getXLScolumn(coloffset-13), i) ...
                sprintf('=SMALL($%s$%d:$%s$%d,ROW()-1)',GC_getXLScolumn(coloffset),2, GC_getXLScolumn(coloffset), maxrows) ...
                sprintf('=IF(ISNUMBER(%s%d),INDEX(%s$%d:%s$%d,MATCH(%s%d,%s$%d:%s$%d,0)),NA())',GC_getXLScolumn(coloffset+1),i, ...
